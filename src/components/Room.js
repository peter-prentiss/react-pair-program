...
class Room extends Component {
  ...
  componentDidMount() {
    if (this.props.challenge.id == undefined) {
      this.prpos.actions.getChallenges();
    } else {
  ...
  sendUsersAndCode() {
    socket.emit('sendUsersAndCode', {room: this.props.challenge.id, users: this.state.users, code: this.state.code});
  }
...
function mapStateToProps(state, ownProps) {
  if (state.challenges.length > 0) {
    const challenge = state.challenges.filter(challenge => {return challenge.id == ownProps.param.id})[0];
